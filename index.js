...
bot.on('guildMemberAdd', async (membro) => {
    ...
    if (canalWelc) {
        const embedWelcome = new EmbedBuilder()
            .setDescription(
                `Oii, ${membro}!O muito obrigada por fazer parte do nosso cantinho! :3\n\n` +
...
// COMANDO PARA MONTAR O GUIA REVISADO NO CANAL #ANEMONA
  ...
  if (cmd === "setup-guia") {
      ...
      const embedGuia = new EmbedBuilder()
          ...
          .setDescription(
              "**• Diversão e Jogos**\n" +
              "`.akinator` — Inicia o jogo do gênio Akinator.\n" +
              "`.forca` — Começa uma partida do jogo da forca.\n" +
              "`.chute [letra]` — Dá um palpite de letra na forca ativa.\n" +
              "`.adivinhar [número]` — Tenta acertar o número secreto de 1 a 50.\n" +
              "`.vd [@membro]` — Joga Verdade ou Desafio.\n" +
              "`.jogos` — Mostra o guia de como jogar Gartic, Uno e Mudae no servidor.\n\n" +
              "**• Laços e Separações**\n" +
  ...
  if (cmd === "musica") {
      ...
      try {
          const conexao = joinVoiceChannel({ channelId: canalVoz.id, guildId: message.guild.id, adapterCreator: mensagem.guild.voiceAdapterCreator });
  ...
  if (cmd === "cor") {
      ...
      const corEscolhida = args?.toLowerCase(); 
      if (!corEscolhida || !CORES_BOOSTER[corEscolhida]) return mensagem.reply("Cores válidas: rosa, azul, roxo, preto, verde, amarelo, vermelho");
  ...
  if (cmd === "banner") {
      ...
      try {
          ...
          if (!urlBanner) return mensagem.reply("Não possui banner."); 
          return mensagem.reply(`Banner de **${usuario.username}**:\n${urlBanner}`);
      } catch (e) { 
          return mensagem.reply("Erro ao buscar."); 
      }
  }

  if (cmd === "roblox") {
      const nomeRoblox = args.join(" ").trim(); 
      if (!nomeRoblox) return mensagem.reply("Digite o nome do Roblox.");
      const embed = new EmbedBuilder()
          .setDescription(`🎮 **Skin do Roblox de:** ${nomeRoblox}`)
          .setImage(`https://roblox.com{nomeRoblox.replace(/[^a-zA-Z0-9]/g, "")}`);
      return mensagem.channel.send({ embeds: [embed] });
  }

  // ==========================================
  // JOGOS E ENTRETENIMENTO REVISADO
  // ==========================================
  if (cmd === "akinator") {
      return mensagem.reply("🔮 **Akinator Iniciado!** Pense em um personagem. Digite `.chute` para desafiar meus palpites!");
  }

  if (cmd === "jogos") {
      const embedJogos = new EmbedBuilder()
          .setTitle("🎮 CENTRAL DE JOGOS")
          .setDescription(
              "• ✏️ **Gartic:** Quer desenhar com a galera? Vai lá no canal `#gartic` e digita `g!jogar` para começar a partida!\n" +
              "• 🃏 **Uno Online:** [Clique aqui para jogar Uno](https://letsplayuno.com)\n" +
              "• 🌸 **Mudae Bot:** Quer roletar Waifu? Vai lá no canal `#mudae` e digita `$wa`!\n" +
              "• 🎧 **Jockie Music:** Quer ouvir um som? Entra numa sala de voz, vai no canal `#cmd` e digita `m!play` seguido do nome da música!"
          )
          .setColor("#ffb6c1");
  ...
  if (cmd === "chute") {
      ...
      if (chu.length > 1) {
          if (chu === j.palavra) { 
              ...
              return mensagem.reply(`ACERTOU! Era **${j.palavra.toUpperCase()}**!`); 
          }
          j.vidas -= 2; 
          if (j.vidas <= 0) { 
              jogosForca.delete(guildId); 
              return mensagem.reply(`Fim! Era: **${j.palavra.toUpperCase()}**.`); 
          } 
          return mensagem.reply(`Errou! Restam: ${j.vidas} vidas.`);
      }
      
      if (j.letrasChutadas.includes(chu)) return mensagem.reply("Já tentada."); 
      ...
      if (j.palavra.includes(chu)) { 
          ...
          mensagem.reply(`Errou! Restam: ${j.vidas} vidas.`); 
      } 
      
      let pf = ''; 
      let g = true; 
      for (const l of j.palavra) { 
          if (j.letrasChutadas.includes(l)) { pf += l.toUpperCase() + ' '; } 
          else { pf += "❓ "; g = false; } 
      }
      if (g) { 
          jogosForca.delete(guildId); 
          return mensagem.channel.send(`GANHOU! Era **${j.palavra.toUpperCase()}**!`); 
      }
      return mensagem.channel.send(`Palavra: \`${pf.trim()}\` | Tentadas: [ ${j.letrasChutadas.map(l => l.toUpperCase()).join(', ')} ]`);
  }

  // ==========================================
  // 💬 RESPOSTAS INFORMAIS E SINCERAS (.PERGUNTA)
  // ==========================================
  if (cmd === "pergunta") {
      const texto = args.join(" ").toLowerCase(); 
      if (!texto) return mensagem.reply("O que quer perguntar?");
      
      if (texto.includes("hello") || texto.includes("hi ") || texto.includes("hola")) {
          return mensagem.reply("No hablo gringo não, parceiro! 🤣");
      }
  ...
  // ==========================================
  // 🧹 MODERAÇÃO: LIMPEZA DE CHAT (.CL E .CLEARALL)
  // ==========================================
  if (cmd === "cl") {
      ...
      if (ehBooster && !mensagem.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
          ...
          try { await mensagem.channel.bulkDelete(quantidade + 1, true); } catch (e) {} 
          return;
      }
...
// Inicialização segura na nuvem usando as chaves secretas do Render
bot.login(MTUzMzI2OTUwMTAzMjkyNzMwMg.GAnlU8.rp6KIKIlqc0JE0semzWb2SHxAVl3U1IvWwxWoI);
