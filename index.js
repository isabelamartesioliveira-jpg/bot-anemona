...
// ==========================================
// 👋 SISTEMA DE BOAS-VINDAS ATUALIZADO
// ==========================================
...
bot.on('messageCreate', async (mensagem) => {
    ...
    // Sistema do Espelho de Mensagens
    const canalMensagens = mensagem.guild?.channels.cache.find(c => c.name.toLowerCase() === 'mensagens' && c.type === ChannelType.GuildText);
    ...
    // Sistema de Segredos Anônimos (#hidden)
...
if (cmd === "velorio") {
    const alvo = mensagem.mentions.users.first(); 
    if (!alvo) return mensagem.reply("Mencione quem faleceu.");
...
// ==========================================
if (cmd === "avatar") {
    // Pega o membro mencionado ou usa o autor da própria mensagem
...
if (cmd === "banner") {
    const usuario = mensagem.mentions.users.first() || mensagem.author;
    try {
        // Força a busca detalhada no Discord para encontrar o arquivo de banner oculto
...
if (cmd === "roblox") {
    ...
    // Filtra e limpa o texto para evitar nomes inválidos
...
if (cmd === "musica") {
    // Verifica se quem digitou o comando está em algum canal de voz
    ...
    // Importa as ferramentas de áudio oficiais do Discord
    ...
    try {
        // Conecta o robô no canal de voz atual do membro
        const conexao = joinVoiceChannel({ 
            channelId: canalVoz.id, 
            guildId: mensagem.guild.id, 
            adapterCreator: mensagem.guild.voiceAdapterCreator 
        });

        // Cria o reprodutor de música em segundo plano
        const reprodutor = createAudioPlayer(); 
        
        // Conecta o link da transmissão de streaming de lofi externa
        const radioStream = createAudioResource("https://zeno.fm"); 
        
        // Começa a tocar e vincula o reprodutor à conexão da call
        reprodutor.play(radioStream); 
        conexao.subscribe(reprodutor);
    } catch (e) {
        // Caso ocorra algum erro na conexão de voz, o bot ignora silenciosamente para não cair
    }
    return;
}
// ==========================================
// SISTEMAS DE MINIJOGOS (ADIVINHAR E FORCA)
// ==========================================
if (cmd === "adivinhar") {
    const palpite = parseInt(args[0]); 
    const guildId = mensagem.guild.id;

    // Se não houver um jogo ativo nesse servidor, cria um número novo de 1 a 50
...
if (cmd === "forca") {
    ...
    // Sorteia uma palavra secreta da nossa lista
    ...
    // Transforma todas as letras em emojis de interrogação "❓"
...
if (cmd === "chute") {
    ...
    const chu = args[0]?.toLowerCase();
    if (!chu) return mensagem.reply("Digite uma letra para dar o seu chute!");
    
    const j = jogosForca.get(guildId);

    // Se o jogador chutar mais de uma letra (tentando adivinhar a palavra inteira de uma vez)
    if (chu.length > 1) {
        if (chu === j.palavra) { 
            jogosForca.delete(guildId); 
            return mensagem.reply(`🎉 **ACERTOU A PALAVRA INTEIRA!** Era **${j.palavra.toUpperCase()}**!`); 
        }
        
        j.vidas -= 2; // Errar a palavra inteira custa 2 vidas
        if (j.vidas <= 0) { 
            jogosForca.delete(guildId); 
            return mensagem.reply(`💀 **Fim de jogo! Suas vidas acabaram.** A palavra correta era: **${j.palavra.toUpperCase()}**.`); 
        } 
        return mensagem.reply(`❌ Palavra errada! Restam: ${j.vidas} vidas.`);
    }

    // Se o jogador chutar uma única letra
    if (j.letrasChutadas.includes(chu)) return mensagem.reply("Essa letra já foi tentada antes, escolha outra!"); 
    j.letrasChutadas.push(chu);

    if (j.palavra.includes(chu)) { 
        mensagem.reply(`✅ A letra \`${chu.toUpperCase()}\` está na palavra.`); 
    } else { 
        j.vidas--; 
        if (j.vidas <= 0) { 
            jogosForca.delete(guildId); 
        ...
        mensagem.reply(`❌ Letra errada! Restam: ${j.vidas} cores/vidas.`); 
    } 

    // Monta o visual atualizado da palavra trocando interrogações pelas letras acertadas
    let pf = ''; 
    let ganhou = true; 
    for (const l of j.palavra) { 
        if (j.letrasChutadas.includes(l)) { 
            pf += l.toUpperCase() + ' '; 
        } else { 
            pf += "❓ "; 
            ganhou = false; 
    ...
    if (ganhou) { 
        jogosForca.delete(guildId); 
        return mensagem.channel.send(`🎉 **GANHOU O JOGO!** A palavra era **${j.palavra.toUpperCase()}**!`); 
    } 
...
if (cmd === "pergunta") {
    ...
    if (texto.includes("hello") || texto.includes("hi ") || texto.includes("hola")) {
        return mensagem.reply("No hablo gringo não, parceiro! KKKKKKKKKKKK");
    }
    
    if (texto.includes("+") || texto.includes("-") || texto.includes("*") || texto.includes("/")) {
        return mensagem.reply("Faz de cabeça aí, preguiça!");
    }
    ...
    const opinioes = [
        ...
        "Sua lógica foi tão pesada que criou uma distorção no espaço-tempo e me mandou direto para outra galáxia. 144",
        ...
        "Se a sua inteligência dependesse da profundidade dessa pergunta, você morreria afogado em uma tampinha de garrafa.",
...
if (cmd === "cl") {
    ...
    // EXCEÇÃO DO DONO: Se for você, ignora todas as travas de limite abaixo
    ...
    // Se for um membro Booster comum (sem cargo de Moderação)
    ...
    // Se quem usou NÃO for da Staff (e como passou pela checagem acima, também não é o dono)
    if (!mensagem.member.permissions.has(PermissionFlagsBits.ManageMessages)) return;
    
    // Regra da Staff: limite máximo de 100 mensagens
...
if (cmd === "clearall") {
    // Comando exclusivo seu através do ID do Dono salvo no topo do arquivo
    if (mensagem.author.id !== ID_DONO) return;
    
    try { 
        // Clona a sala inteira na mesma posição e deleta a sala velha cheia de flood
...
// ==========================================
// 🎨 PRIVILÉGIO BOOSTER: CONFIGURAÇÕES E BANCO DE CORES
// ==========================================
// Adicione este objeto no TOPO do seu arquivo index.js, junto com os outros mapas/listas:
const CORES_BOOSTER = {
...
// Cole este comando dentro do evento 'messageCreate', junto com os outros ifs:
if (cmd === "cor") {
    // Bloqueia o uso se o membro não for Booster e também não for você (Dono)
    if (!ehBooster && mensagem.author.id !== ID_DONO) {
        return mensagem.reply("❌ Comando exclusivo para Boosters!");
    }

    // Pega o que foi escrito, junta com hífen se usarem espaço e deixa tudo minúsculo
    ...
    try {
        ...
        // Remove os cargos de cores anteriores do membro para não acumular na lista
        ...
        // Se o cargo para essa cor específica ainda não existir no servidor, o bot cria do zero
        if (!cargoExistente) {
        ...
        // Adiciona o novo cargo de cor ao membro
        await mensagem.member.roles.add(cargoExistente);
        return mensagem.reply(`✨ Cor updated para **${corEscolhida}**!`);
    } catch (e) {
...
bot.on('guildMemberUpdate', async (antigoMembro, novoMembro) => {
    // Verifica se o membro não era Booster antes, mas se tornou agora
    const deuBoostAgora = antigoMembro.premiumSince === null && novoMembro.premiumSince !== null;

    if (deuBoostAgora) {
        const guild = novoMembro.guild;

        // 1. Procura o canal geral ou principal de chat para avisar o servidor
        ...
        if (canalChat) {
            ...
            // Puxa o GIF da pasta gifs/boost
        ...
        // 2. Cria a sala privada temporária válida por 5 horas exatas
        try {
            ...
            // Programa a exclusão automática após 5 horas (5 * 60 * 60 * 1000 milissegundos)
            setTimeout(async () => {
                await salaBooster.delete().catch(() => {});
            }, 18000000);

        } catch (e) {
            console.log("Erro ao tentar criar o canal de Booster.");
        }
    }
});
